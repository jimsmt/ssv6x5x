...
#ifdef SSV_HCI_RESOURCE_FULL_CNT
static int  js_hw_resource_full_cnt = 100;
#endif
...
static int ssv6xxx_hci_txq_flush(struct ssv6xxx_hci_ctrl *hci_ctrl)
{
    ...
    for (txqid = 0; txqid <= SSV_SW_TXQ_ID_MNG; txqid++) {
        ...
        while((skb = skb_dequeue(&sw_txq->qhead))) {
            ...
            if(atomic_read(&hci_ctrl->sw_txq_cnt)>0){
...
#ifdef SSV_HCI_RESOURCE_FULL_CNT              
                if (time_after(jiffies, (hci_ctrl->js_tx_run_time + msecs_to_jiffies(1000)))) {
                    js_tx_result = (( hci_ctrl->js_tx_run_size  << 3) / jiffies_to_msecs(jiffies - hci_ctrl->js_tx_run_time));
                    //printk("js2 debug data[%ld] tx throughput %ld Kbps\n", hci_ctrl->js_tx_run_size, js_tx_result);

                    hci_ctrl->js_tx_run_time = jiffies;
                    hci_ctrl->js_tx_run_size = 0;
                    if(js_tx_result>20000){
                        js_hw_resource_full_cnt = 100;
                    }else{
                        js_hw_resource_full_cnt = 20;
                    }
                }else{
                    hci_ctrl->js_tx_run_size += p_aggr_skb->len;
                }
#endif 
                ret = IF_SEND(hci_ctrl, (void *)p_aggr_skb, p_aggr_skb->len, sw_txq->txq_no);
                if (ret < 0) {
...
static int ssv6xxx_hci_xmit(struct ssv6xxx_hci_ctrl *hci_ctrl, struct ssv_sw_txq *sw_txq, int max_count)
{
    ...
    for(tx_count=0; tx_count<max_count; tx_count++) {
        ...
        for(retry_count = 0; retry_count < 3; retry_count++) {
                ...
                if (time_after(jiffies, (hci_ctrl->js_tx_run_time + msecs_to_jiffies(1000)))) {
                    js_tx_result = (( hci_ctrl->js_tx_run_size  << 3) / jiffies_to_msecs(jiffies - hci_ctrl->js_tx_run_time));
                    //printk("js1 debug data[%ld] tx throughput %ld Kbps\n", hci_ctrl->js_tx_run_size, js_tx_result);
                    ...
                    hci_ctrl->js_tx_run_size += skb->len;
                }
#endif 
            ret = IF_SEND(hci_ctrl, (void *)skb, skb->len, sw_txq->txq_no);
            if (ret < 0) {
    ...
    for(tx_count = 0; tx_count < (*p_max_count); tx_count++) {
        ...
        if ((hw_resource.free_tx_page < page_count) || (hw_resource.free_tx_id <= 0)) {
            ...
            if (0 == tx_count) {
                hci_ctrl->hw_tx_resource_full_cnt++;
#ifdef  SSV_HCI_RESOURCE_FULL_CNT
                if (hci_ctrl->hw_tx_resource_full_cnt > js_hw_resource_full_cnt) {
#else
                if (hci_ctrl->hw_tx_resource_full_cnt > MAX_HW_RESOURCE_FULL_CNT) {
#endif
                    hci_ctrl->hw_tx_resource_full = true;
