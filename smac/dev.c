...
static void ssv6xxx_proc_data_rx_skb (struct ssv_softc *sc, struct sk_buff *rx_skb)
{
    ...
    if(ieee80211_is_beacon(hdr->frame_control))
    {
        ...
        //printk("bssid=%pM\r\n",hdr->addr3);
        if (ssv6xxx_find_sta_by_addr(sc, hdr->addr3) != NULL)
        {
            is_ap = true;
        }
    ...
    if (is_ap)
    {
        if (sc->cur_channel != NULL)
        {
            rxs->band = sc->cur_channel->band;
            rxs->freq = sc->cur_channel->center_freq;
        }
        
        //printk("bssid=%pM, band=%d, freq=%d \n",hdr->addr3, rxs->band, rxs->freq);
    }
